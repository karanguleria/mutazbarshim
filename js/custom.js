...
jQuery('.btn-secondary.cart').click(function (e) {
    ...
    jQuery.post('http://localhost/mystaticsite/api/cart/add/', {stProduct: product, stQuantity: qty, stVariation: variation}, function (data, status) {
...
jQuery('.btn-secondary.buy').click(function (e) {
    ...
    jQuery.post('http://localhost/mystaticsite/api/cart/buy/', {stProduct: product, stQuantity: qty, stVariation: variation}, function (data, status) {
        console.log(data);
        var obj = JSON.parse(data);
        if ($("input[name='cart[" + obj.key + "][qty]']").length)
        {
